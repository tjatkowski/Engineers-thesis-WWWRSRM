...
public class CoordinatesConverter {
    ...
    public static double convertLongitudeToX(double longitude, int zoomLevel) {
        return ((longitude + 180) / 360) * 256 * Math.pow(2, zoomLevel);
    }

    public static double convertLatitudeToY(double latitude, int zoomLevel) {
        double sinLatitude = Math.sin(latitude * Math.PI / 180);
        return (0.5 - Math.log((1 + sinLatitude) / (1 - sinLatitude)) / (4 * Math.PI)) * 256 * Math.pow(2, zoomLevel);
    }

    public static double convertXToLongitude(double x, int zoomLevel) {
        return ((x / (256 * Math.pow(2, zoomLevel))) - 0.5) * 360;
    }

    public static double convertYToLatitude(double y, int zoomLevel) {
     ...
     */
    public static double scaleXToFitWindow(double x, Point2D minBound, Point2D maxBound, int mapWidth, int zoomLevel) {
     ...
     */
    public static double scaleYToFitWindow(double y, Point2D minBound, Point2D maxBound, int mapHeight, int zoomLevel) {
